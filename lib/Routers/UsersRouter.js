...
var UsersRouter = exports.UsersRouter = function (_ClassesRouter) {
  ...
  _createClass(UsersRouter, [{
    ...
    value: function handleMe(req) {
      ...
      return _rest2.default.find(req.config, _Auth2.default.master(req.config), '_Session', { _session_token: req.info.sessionToken }, { include: 'user' }).then(function (response) {
    ...
    value: function getExpressRouter() {
      ...
      router.route('GET', '/users', function (req) {
        return _this2.handleFind(req);
      });
      router.route('POST', '/users', function (req) {
        return _this2.handleCreate(req);
      });
      router.route('GET', '/users/:objectId', function (req) {
        return _this2.handleGet(req);
      });
      router.route('PUT', '/users/:objectId', function (req) {
        return _this2.handleUpdate(req);
      });
      router.route('DELETE', '/users/:objectId', function (req) {
        return _this2.handleDelete(req);
      });
      router.route('GET', '/users/me', function (req) {
        return _this2.handleMe(req);
      });
      router.route('GET', '/login', function (req) {
        return _this2.handleLogIn(req);
      });
      router.route('POST', '/logout', function (req) {
        return _this2.handleLogOut(req);
      });
      router.route('POST', '/requestPasswordReset', function () {
